...
export class CreateUsersTableOnly1762006166000 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        // Drop policies
        await queryRunner.query(`DROP POLICY IF EXISTS "users_select_own" ON "users"`);
        await queryRunner.query(`DROP POLICY IF EXISTS "users_update_own" ON "users"`);
        await queryRunner.query(`DROP POLICY IF EXISTS "users_insert_own" ON "users"`);
        await queryRunner.query(`DROP POLICY IF EXISTS "users_delete_none" ON "users"`);
        await queryRunner.query(`DROP POLICY IF EXISTS "users_full_access_admin" ON "users"`);
